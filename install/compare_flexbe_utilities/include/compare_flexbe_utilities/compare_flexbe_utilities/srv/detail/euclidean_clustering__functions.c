...
compare_flexbe_utilities__srv__EuclideanClustering_Request__create(void)
{
...
compare_flexbe_utilities__srv__EuclideanClustering_Response__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
...
bool
compare_flexbe_utilities__srv__EuclideanClustering_Event__init(compare_flexbe_utilities__srv__EuclideanClustering_Event * msg)
{
  if (!msg) {
    return false;
  }
  ...
  if (!service_msgs__msg__ServiceEventInfo__init(&msg->info)) {
    compare_flexbe_utilities__srv__EuclideanClustering_Event__fini(msg);
    return false;
  }
  // request
  if (!compare_flexbe_utilities__srv__EuclideanClustering_Request__Sequence__init(&msg->request, 0)) {
    compare_flexbe_utilities__srv__EuclideanClustering_Event__fini(msg);
    return false;
  }
  // response
  if (!compare_flexbe_utilities__srv__EuclideanClustering_Response__Sequence__init(&msg->response, 0)) {
    compare_flexbe_utilities__srv__EuclideanClustering_Event__fini(msg);
    return false;
  }
  return true;
...
void
compare_flexbe_utilities__srv__EuclideanClustering_Event__fini(compare_flexbe_utilities__srv__EuclideanClustering_Event * msg)
{
  if (!msg) {
    return;
  }
  ...
  // request
  compare_flexbe_utilities__srv__EuclideanClustering_Request__Sequence__fini(&msg->request);
  // response
  compare_flexbe_utilities__srv__EuclideanClustering_Response__Sequence__fini(&msg->response);
}

bool
compare_flexbe_utilities__srv__EuclideanClustering_Event__are_equal(const compare_flexbe_utilities__srv__EuclideanClustering_Event * lhs, const compare_flexbe_utilities__srv__EuclideanClustering_Event * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  ...
  // request
  if (!compare_flexbe_utilities__srv__EuclideanClustering_Request__Sequence__are_equal(
      &(lhs->request), &(rhs->request)))
  {
    return false;
  }
  // response
  if (!compare_flexbe_utilities__srv__EuclideanClustering_Response__Sequence__are_equal(
...
bool
compare_flexbe_utilities__srv__EuclideanClustering_Event__copy(
  const compare_flexbe_utilities__srv__EuclideanClustering_Event * input,
  compare_flexbe_utilities__srv__EuclideanClustering_Event * output)
{
  if (!input || !output) {
    return false;
  }
  ...
  // request
  if (!compare_flexbe_utilities__srv__EuclideanClustering_Request__Sequence__copy(
      &(input->request), &(output->request)))
  {
    return false;
  }
  // response
  if (!compare_flexbe_utilities__srv__EuclideanClustering_Response__Sequence__copy(
...
compare_flexbe_utilities__srv__EuclideanClustering_Event__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  compare_flexbe_utilities__srv__EuclideanClustering_Event * msg = (compare_flexbe_utilities__srv__EuclideanClustering_Event *)allocator.allocate(sizeof(compare_flexbe_utilities__srv__EuclideanClustering_Event), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(compare_flexbe_utilities__srv__EuclideanClustering_Event));
  bool success = compare_flexbe_utilities__srv__EuclideanClustering_Event__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
...
void
compare_flexbe_utilities__srv__EuclideanClustering_Event__destroy(compare_flexbe_utilities__srv__EuclideanClustering_Event * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    compare_flexbe_utilities__srv__EuclideanClustering_Event__fini(msg);
...
bool
compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence__init(compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  compare_flexbe_utilities__srv__EuclideanClustering_Event * data = NULL;

  if (size) {
    data = (compare_flexbe_utilities__srv__EuclideanClustering_Event *)allocator.zero_allocate(size, sizeof(compare_flexbe_utilities__srv__EuclideanClustering_Event), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = compare_flexbe_utilities__srv__EuclideanClustering_Event__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        compare_flexbe_utilities__srv__EuclideanClustering_Event__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
...
void
compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence__fini(compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence * array)
{
  if (!array) {
    return;
  }
  ...
  if (array->data) {
    ...
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      compare_flexbe_utilities__srv__EuclideanClustering_Event__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
...
compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence * array = (compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence *)allocator.allocate(sizeof(compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
...
void
compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence__destroy(compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence__are_equal(const compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence * lhs, const compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  ...
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!compare_flexbe_utilities__srv__EuclideanClustering_Event__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
...
bool
compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence__copy(
  const compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence * input,
  compare_flexbe_utilities__srv__EuclideanClustering_Event__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(compare_flexbe_utilities__srv__EuclideanClustering_Event);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    compare_flexbe_utilities__srv__EuclideanClustering_Event * data =
      (compare_flexbe_utilities__srv__EuclideanClustering_Event *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    ...
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!compare_flexbe_utilities__srv__EuclideanClustering_Event__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          compare_flexbe_utilities__srv__EuclideanClustering_Event__fini(&output->data[i]);
        }
        return false;
      }
    }
  ...
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!compare_flexbe_utilities__srv__EuclideanClustering_Event__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
